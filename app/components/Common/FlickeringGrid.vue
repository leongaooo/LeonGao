<template>
  <div
    ...
    aria-hidden="true"
  >
    <canvas ref="canvasRef" absolute top-0 left-0 w-full h-full z-1></canvas>
    <div class="absolute top-0 left-0 w-full h-full z-3">
      <slot> </slot>
    </div>
  </div>
</template>
...
onMounted(() => {
  ...
  setTimeout(() => {
    ...
    const height = `${article?.clientHeight || 0}px`;
    canvasContainerRef.value!.style.height = height;
    initCanvas();
  }, 1500);
});
